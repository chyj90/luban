...
**目标**：补充当前缺失的应用级和页面级权限控制。现有权限体系只覆盖了平台全局页面（菜单显隐），但缺少"谁能访问哪个应用"和"应用内每个页面谁能看"的粒度。
...
| 平台权限 | ✅ 角色 → 17 个菜单 Key | — |
| 问数权限 | ✅ 角色 → 概念域 | — |
| API Key 权限 | ✅ Key → 工具/数据源 | — |
| **应用权限** | ❌ 无 | 任何人都能看到所有应用 |
| **页面权限** | ❌ 无 | 应用内所有页面对所有人可见 |

**场景**：财务部建了一个"薪资分析"应用，只有 HR 和财务总监能访问，应用内的"个人薪资明细"页面只有 HR 能看。

**实现路径**：

```
Phase 1：应用成员管理
  · 创建应用时可指定成员（用户/角色），支持增删改
  · 应用列表按成员过滤，非成员不可见
  · 前端 Router 守卫：非成员访问应用路由 → 重定向到 403

Phase 2：页面级权限
  · 每个页面配置可见角色（在页面编辑器中设置）
  · 左侧页面树按权限过滤，无权限的页面不显示
  · 直接 URL 访问无权限页面 → 403

Phase 3：权限继承与联动
  · 问数权限自动继承：应用成员自动获得该应用关联概念域的查询权限
  · 页面权限联动：页面绑定的数据源权限与页面可见性同步
```

**验证方法**：创建 3 个应用，各分配不同成员和页面权限。用 5 个不同角色账号登录，验证每个账号看到的应用列表和页面树是否符合预期，误漏率目标 0%。

---

### 五、流程权限（审批人校验 + 流程定义权限 + 启动/运维权限）

**目标**：补全流程引擎的权限控制。当前流程引擎功能完整（审批节点、协作模式、驳回、撤回、冻结），但所有接口均无权限校验，任何认证用户都能操作任何流程。

**现状差距**：

| 缺口 | 问题 | 代码位置 |
|------|------|------|
| 审批人校验 | `completeTask` 不校验 operatorId 是否在任务 allAssigneeIds 中，任何人可审批 | ProcessEngine:L125 |
| 流程定义权限 | create/update/publish/delete/copy 无权限检查 | ProcessController:L37-68 |
| 流程启动权限 | 任何认证用户可启动任意流程定义 | WorkflowInstanceController:L22 |
| 流程运维权限 | cancel/freeze/unfreeze 不校验是否为发起人 | WorkflowInstanceController |

**实现路径**：

```
Phase 1：审批人校验（最紧急，安全漏洞）
  · completeTask 入口处校验 operatorId 是否在 allAssigneeIds 中
  · 转办/加签操作不需要校验（本身就是合法操作）
  · 增加审批人列表变更日志（谁加了谁、谁转了谁）

Phase 2：流程定义权限
  · 流程定义归属应用：只有应用成员可创建/编辑该应用的流程
  · 发布权限：只有应用管理员或平台管理员可发布
  · 前端：流程设计器页面按应用权限入口控制

Phase 3：流程实例权限
  · 启动权限：只有应用成员可启动流程
  · 取消/冻结：只有发起人或管理员可操作
  · 查看权限：按角色分层（发起人看全部、审批人看审批过的、管理员看全部）
```

**验证方法**：用 3 个不同角色用户（非任务审批人、非流程发起人）直接调用后端 API，验证审批/取消/删除/发布操作是否返回 403。安全扫描：遍历所有流程 API，确认每个接口都有授权判断。
