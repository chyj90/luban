...
2. 确认表结构是否满足需求：
   ...
   - ⚠️ **人工介入契约（必须遵守，判定只认状态行 JSON，不认散文）**：凡是需要用户手动执行 SQL/DDL 才能继续的情况（无论"尝试后被拦截"还是"需求明确禁止尝试"），最终汇报**必须以状态行结尾**：
...
2. fetch_datasource_structure 确认目标表是否存在及现有列：
   - 表不存在 → 先走建表流程（execute_sql，被拦截时按「人工介入契约」在正文输出 DDL 并以状态行标记 interventionRequired: true），等表建好再继续
...
- 若 execute_sql 执行 ALTER TABLE 被后端拦截，则生成 ALTER TABLE SQL 告知用户手动执行（DDL 拦截是确定行为，被拦后禁止再重试或换写法尝试，等待用户确认即可）
