...
export function EditorSidebar({ appId, currentPageId, pages, selectedQuery, activeTab: controlledActiveTab, workflowView, queries, onQueriesChange, onPageChange, onPagesChange, onQuerySelect, onWorkflowNavigate, onTabChange, selectedApi, onApiSelect, onToolsChange, toolsVersion, pendingApiId, onPendingApiHandled }: EditorSidebarProps) {
  ...
  const handleResetPage = async (e: React.MouseEvent, page: Page) => {
    e.stopPropagation();
    setMenuOpen(null);
    const confirmed = await confirm({
      title: '恢复为组件库',
      message: `将页面「${page.name}」恢复为 LubanUI 组件库页面？当前代码将被覆盖。`,
      confirmText: '恢复',
      variant: 'default',
    });
    if (!confirmed) return;
    try {
      await updateCodePage(page.id, {
                  ...
                  <div className="editor-sidebar-item-menu" ref={menuOpen === page.id ? menuRef : null}>
                    ...
                    {menuOpen === page.id && (
                      <div className="editor-sidebar-item-dropdown">
                        <button onClick={(e) => handleRenameStart(e, page)}>
                        ...
                        <button onClick={(e) => handleResetPage(e, page)}>
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
