...
**大屏类需求建议以 create_page_scaffold(type='dashboard') 为起点（省去环境配置），但骨架的区块排布只是质感基线的示例——面板数量/栅格比例/有无地图必须按需求分析第 4 章的模块构成灵活重组，禁止每个大屏都套同一布局。**
...
| 需求域 | 主视图范式 | 典型布局 |
|--------|-----------|---------|
| 地理态势/站点分布 | 中央 GIS 地图（LubanUI.gis，默认优先） | KPI行 + 左右面板 + 中央GIS |
| 省份级填色统计（choropleth） | 逻辑地图（LubanUI.map，GIS 的唯一常规例外之一） | KPI行 + 左右面板 + 中央地图 |
