...
env | grep -E '^(LUBAN_|SPRING_|MYSQL_|EMBEDDING_|HF_|APP_)' > /etc/luban.env
...
# 用户把 tls.crt + tls.key 放进 /app/data/ssl/ 即可（与数据盘同一 volume，无需额外 -v）
if [ -f "$SSL_SRC/tls.crt" ] && [ -f "$SSL_SRC/tls.key" ]; then
    cp "$SSL_SRC/tls.crt" "$SSL_SRC/tls.key" "$NGINX_SSL_DIR/"
    echo "[entry] 检测到 /app/data/ssl/ 中的 TLS 证书，启用 HTTPS"
...
NGINX_EOF
    ...
    echo "[entry] 未检测到 TLS 证书，使用 HTTP-only 配置"
    echo "[entry] 如需 HTTPS，将 tls.crt + tls.key 放入 /app/data/ssl/ 即可"
fi
